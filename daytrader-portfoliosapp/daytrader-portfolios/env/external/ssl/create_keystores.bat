...
rem
rem This script was run to create the keystores. It is called via an off-line
rem administrative action and then the resulting keystores are copied into the 
rem src\main\resources folder. This script documents the set of commands that 
rem were used to make it easier for others.
rem
...
keytool -keystore keystore.jks -storepass password -certreq -alias server | keytool -keystore ca.jks -storepass password -gencert -alias ca -ext ku:c=dig,keyEnc -ext san=dns:localhost -ext eku=sa,ca -rfc > server.pem
...
rem import server cert chain into truststore.jks
